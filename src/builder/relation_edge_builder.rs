...
impl RelationEdgeBuilder {
    pub fn new<S: Into<String>>(outbound_id: Uuid, type_name: S, inbound_id: Uuid) -> RelationEdgeBuilder {
        RelationEdgeBuilder {
            outbound_id,
    ...
    pub fn property<'a, S: Into<String>>(&'a mut self, property_name: S, value: Value) -> &'a mut RelationEdgeBuilder {
        self.properties.insert(property_name.into(), value);
        self
    }

    pub fn create<'a>(&'a mut self, relation_edge_manager: Arc<dyn RelationEdgeManager>) -> Result<EdgeKey, RelationEdgeCreationError> {
        let edge_key = EdgeKey::new(self.outbound_id, Type::new(self.type_name.clone()).unwrap(), self.inbound_id);
