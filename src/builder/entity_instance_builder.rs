...
use crate::api::{EntityInstanceManager, EntityInstanceCreationError};
...
impl EntityInstanceBuilder {
    ...
    pub fn property<'a, S: Into<String>>(&'a mut self, property_name: S, value: Value) -> &'a mut EntityInstanceBuilder {
        self.properties.insert(property_name.into(), value);
        self
    }

    pub fn create<'a>(&'a mut self, entity_instance_manager: Arc<dyn EntityInstanceManager>) -> Result<Uuid, EntityInstanceCreationError> {
        if self.id.is_some() {
            entity_instance_manager.create_with_id(self.type_name.clone(), self.id.unwrap(), self.properties.clone())
        } else {
