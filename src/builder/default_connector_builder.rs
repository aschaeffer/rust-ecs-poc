...
use crate::model::{RelationInstance, EntityInstance};
...
impl DefaultConnectorBuilder {
    ...
    pub fn outbound<'a, S: Into<String>>(&'a mut self, outbound_entity: EntityInstance, property_name: S) -> &'a mut DefaultConnectorBuilder {
        self.outbound = Some((outbound_entity.id, property_name.into().clone()));
        self
    }

    pub fn inbound<'a, S: Into<String>>(&'a mut self, inbound_entity: EntityInstance, property_name: S) -> &'a mut DefaultConnectorBuilder {
    ...
    pub fn get(&mut self) -> RelationInstance {
        ...
        let type_name = Connector::type_name(DefaultConnector::TYPE_NAME, outbound.1.as_str(), inbound.1.as_str());
        RelationInstanceBuilder::new(outbound.0, type_name, inbound.0)
            .property(ConnectorProperties::OUTBOUND_PROPERTY_NAME.to_string(), json!(outbound.1))
            .property(ConnectorProperties::INBOUND_PROPERTY_NAME.to_string(), json!(inbound.1))
            .get()
    }
