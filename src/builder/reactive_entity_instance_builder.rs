...
use crate::api::{ReactiveEntityInstanceManager, ReactiveEntityInstanceCreationError};
...
impl ReactiveEntityInstanceBuilder {
    ...
    pub fn property<'a, S: Into<String>>(&'a mut self, property_name: S, value: Value) -> &'a mut ReactiveEntityInstanceBuilder {
        self.properties.insert(property_name.into(), value);
        self
    }

    pub fn create<'a>(&'a mut self, reactive_entity_instance_manager: Arc<dyn ReactiveEntityInstanceManager>) -> Result<Arc<ReactiveEntityInstance>, ReactiveEntityInstanceCreationError> {
        if self.id.is_some() {
            reactive_entity_instance_manager.create_with_id(self.type_name.clone(), self.id.unwrap(), self.properties.clone())
        } else {
