...
impl EntityVertexBuilder {
    ...
    pub fn property<'a, S: Into<String>>(&'a mut self, property_name: S, value: Value) -> &'a mut EntityVertexBuilder {
        self.properties.insert(property_name.into(), value);
        self
    }

    pub fn create<'a>(&'a mut self, entity_vertex_manager: Arc<dyn EntityVertexManager>) -> Result<Uuid, EntityVertexCreationError> {
        if self.id.is_some() {
            entity_vertex_manager.create_with_id(self.type_name.clone(), self.id.unwrap(), self.properties.clone())
        } else {
