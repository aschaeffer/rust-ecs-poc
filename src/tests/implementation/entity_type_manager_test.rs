...
fn test_register_entity_type () {
    ...
    let type_name = String::from("test1");
    application.entity_type_manager.register(
...
fn test_create_and_delete_entity_type () {
    ...
    application.entity_type_manager.create(
        ...
        vec![
            PropertyType::new(String::from("x"), String::from("string"))
        ]
    );
    assert!(application.entity_type_manager.has(type_name.clone()));
...
fn test_load_static_entity_types () {
    ...
    assert!(!application.entity_type_manager.has(String::from("asdfjasdjfinasfhibna9iiiasdf")));
}
...
fn test_register_entity_type_has_component () {
    ...
    let component_name = String::from("test1");
    ...
    let entity_type_name = String::from("test1");
    application.entity_type_manager.register(
        crate::model::EntityType::new(
            entity_type_name.clone(),
            vec![ component_name.clone() ],
            vec![
                crate::model::PropertyType::new(String::from("y"), String::from("string"))
            ]
        )
    );
    let entity_type: EntityType = application.entity_type_manager.get(entity_type_name.clone()).unwrap();
...
fn test_export_import_entity_type () {
    ...
    application.entity_type_manager.export(type_name.clone(), path.clone());
    assert!(application.entity_type_manager.has(type_name.clone()));
    application.entity_type_manager.delete(type_name.clone());
    assert!(!application.entity_type_manager.has(type_name.clone()));
    println!("Importing from: {}", path.clone());
    application.entity_type_manager.import(path.clone());
    assert!(application.entity_type_manager.has(type_name.clone()));
}
