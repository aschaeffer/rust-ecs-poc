...
use serde_json::json;
use std::sync::Arc;
use crate::reactive::arithmetic_gate::{create_arithmetic_gate_entity, ArithmeticGate, PROPERTY_NAME_NUMBER_1, PROPERTY_NAME_NUMBER_2, PROPERTY_NAME_RESULT_1};

#[test]
fn and_gate_type_test () {
    let and = Arc::new(create_arithmetic_gate_entity(AddGate::TYPE_NAME.to_string()));
...
fn three_add_gates_test () {
    let add_1 = Arc::new(create_arithmetic_gate_entity(AddGate::TYPE_NAME.to_string()));
    let add_2 = Arc::new(create_arithmetic_gate_entity(AddGate::TYPE_NAME.to_string()));
    let add_3 = Arc::new(create_arithmetic_gate_entity(AddGate::TYPE_NAME.to_string()));
