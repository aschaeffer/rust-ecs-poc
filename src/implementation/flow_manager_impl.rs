use crate::api::{FlowCreationError, FlowImportError, FlowManager, EntityInstanceManager, RelationInstanceManager};
    ...
    fn export(&self, flow: Flow, path: String) {
        ...
        match r_file {
            Ok(file) => {
                ...
                if result.is_err() {
                    error!("Failed to export flow {} to {}: {}",
                           flow.id, path, result.err().unwrap());
                }
            }
            Err(error) => {
                error!("Failed to export flow {} to {}: {}",
                       flow.id, path, error.to_string());
            }
        }
    }
}
